...
public sealed class HabitsController(ApplicationDbContext dbContext, LinkService linkService) : ControllerBase
{
    ...
    { 
        ...
        if (!dataShapingService.Validate<HabitDto>(query.Fields))
        {
            return Problem(
                 statusCode: StatusCodes.Status400BadRequest,
                 detail: $"The provided data shaping field aren't valid: '{query.Fields}'"
                 );
        }
    ...
    public async Task<IActionResult> GetHabit(string id, string? fields, DataShapingService dataShapingService)
    {
        if (!dataShapingService.Validate<HabitWithTagsDto>(fields))
        {
            return Problem(
                 statusCode: StatusCodes.Status400BadRequest,
                 detail: $"The provided data shaping field aren't valid: '{fields}'"
                 );
        }
    ...
    private List<LinkDto> CreateLinksForHabits(HabitQueryParameters parameters,bool hasNextPage,bool hasPreviousPage)
    {
        ...
        if (hasPreviousPage)
        {
            links.Add(linkService.Create(nameof(GetHabits), "prevous-page", HttpMethods.Get, new
            {
                page = parameters.Page - 1,
                pageSize = parameters.PageSize,
                fields = parameters.Fields,
